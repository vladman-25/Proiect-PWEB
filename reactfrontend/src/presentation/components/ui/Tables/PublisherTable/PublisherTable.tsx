...
import { usePublisherTableController } from "./PublisherTable.controller";
import { PublisherDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { PublisherAddDialog } from "../../Dialogs/PublisherAddDialog";
import { PublisherUpdateDialog } from "../../Dialogs/PublisherUpdateDialog";
 ...
 */
const useHeader = (): { key: keyof PublisherDTO, name: string }[] => {
 ...
 */
const getRowValues = (entries: PublisherDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const PublisherTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, handleSearch, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = usePublisherTableController(); // Use the controller hook.
    ...
    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        ...
        <PublisherAddDialog /> {/* Add the button to open the user add modal. */}
                ...
                <TableBody>
                    {
                        rowValues?.map(({ data, entry }, rowIndex) => <TableRow key={`row_${rowIndex + 1}`}>
                            ...
                            <TableCell> {/* Add other cells like action buttons. */}
                                ...
                                {entry.id !== ownUserId && <PublisherUpdateDialog {...entry}/>}
                            </TableCell>
                        </TableRow>)
                    }
                </TableBody>
