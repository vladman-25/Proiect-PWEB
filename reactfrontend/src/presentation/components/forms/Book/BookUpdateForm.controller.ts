import { BookUpdateFormController, BookUpdateFormModel } from "./BookUpdateForm.types";
 ...
 */
const getDefaultValues = (initialData?: BookUpdateFormModel) => {
    const defaultValues = {
        id: "",
        title: "",
        description: "",
...
const mapBookDTOToFormModel = (book: BookDTO): BookUpdateFormModel => { 
    return { id: book?.id ?? '', 
            title: book?.title ?? "",
            ...
            genre: book?.genre ?? "" as BookGenreEnum
        }; };

/**
 * Create a hook to get the validation schema.
 */
const useInitBookUpdateForm = (entry?: BookDTO) => {
    const { formatMessage } = useIntl();

    const defaultValues = getDefaultValues(mapBookDTOToFormModel(entry ?? {}));
 ...
 */
export const useBookUpdateFormController = (onSubmit?: () => void, entry?: BookDTO ): BookUpdateFormController => {
    const { defaultValues, resolver } = useInitBookUpdateForm(entry);
    const { updateBook: { mutation, key: mutationKey }, getBooks: { key: queryKey } } = useBookApi();
    const { mutateAsync: add, status } = useMutation({
        mutationKey: [mutationKey], 
        mutationFn: mutation
    });
    const queryClient = useQueryClient();
    const submit = useCallback((data: BookUpdateFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<BookUpdateFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
