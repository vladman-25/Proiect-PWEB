...
const mapUserDTOToFormModel = (User: UserDTO): UserUpdateFormModel => { 
    return { id: User?.id ?? '', 
            name: User?.name ?? '', 
            email: User?.email ?? '',
            password: "",
 ...
 */
const useInitUserUpdateForm = (entry?: UserDTO) => {
    const { formatMessage } = useIntl();

    const defaultValues = getDefaultValues(mapUserDTOToFormModel(entry ?? {}));
 ...
 */
export const useUserUpdateFormController = (onSubmit?: () => void, entry?: UserDTO ): UserUpdateFormController => {
    const { defaultValues, resolver } = useInitUserUpdateForm(entry);
